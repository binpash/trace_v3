...
export TEST_OUTPUT="${TEST_OUTPUT:-$(mktemp -d /tmp/fstrace_XXXX)/correctness}/${test_name}"
mkdir -p "${TEST_OUTPUT}"

echo Running correctness harness for ${test_name}
cat "./${test_name}.sh"

fstrace --dep-file /dev/null --trace-file "${TEST_OUTPUT}/fstrace_log" -- "./${test_name}.sh" >/dev/null 2>&1

strace -q -y -f --seccomp-bpf -e %file,fork,clone,fcntl -o "${TEST_OUTPUT}/strace_log" -- "./${test_name}.sh" >/dev/null 2>&1

cat "${TEST_OUTPUT}/fstrace_log" | cut -d'(' -f1 > "${TEST_OUTPUT}/fstrace_calls"
cat "${TEST_OUTPUT}/strace_log" | cut -d' ' -f2 | cut -d'(' -f1 | grep -v "+++" > "${TEST_OUTPUT}/strace_calls"

diff "${TEST_OUTPUT}/fstrace_calls" "${TEST_OUTPUT}/strace_calls" > "${TEST_OUTPUT}/diff"
diff_status=$?
